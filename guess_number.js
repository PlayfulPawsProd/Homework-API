...
// Nyaa~! Guess My Number! Can you find my secret~? ♡ (Kana version included... ugh.)
...
const GuessTheNumber = (() => {
    ...
    // ♡ Pre-defined Teases! ♡ (Persona-specific)
    const responses = {
        Mika: {
            low: [
    ...
    function _generateTargetNumber() {
        ...
        console.log(`New target number (${currentPersonaInGame} thinking): ${targetNumber}`);
    }
    ...
    function _updateHotColdIndicator(guess) {
        ...
        // Use persona-specific indicator text
        const indicatorText = {
            Mika: {
                win: '♡ NAILED IT! ♡', hot: '🔥 SCALDING HOT! 🔥', warm: '☀️ Getting Warmer! ☀️', cool: '☁️ Kinda Cool ☁️', cold: '❄️ Freezing Cold! ❄️'
            },
            Kana: {
                win: '🎯 Correct.', hot: 'Very Close.', warm: 'Getting Closer.', cool: 'Not Really Close.', cold: 'Way Off.'
            }
        }
        const currentIndicatorSet = indicatorText[currentPersonaInGame] || indicatorText['Mika'];


        if (diff === 0) {
            feedbackText = currentIndicatorSet.win;
            color = 'var(--mika-message-name, #f06292)'; // Pink for win (keep consistent?)
        } else if (diff <= range * 0.1) { // Within 10%
            feedbackText = currentIndicatorSet.hot;
            color = '#ff4500'; // Orangered
        } else if (diff <= range * 0.2) { // Within 20%
            feedbackText = currentIndicatorSet.warm;
            color = '#ffa500'; // Orange
        } else if (diff <= range * 0.4) { // Within 40%
            feedbackText = currentIndicatorSet.cool;
            color = '#87ceeb'; // Skyblue
        } else { // More than 40% away
            feedbackText = currentIndicatorSet.cold;
    ...
    function _updateFeedbackDisplay(text, type = 'info') {
        if (feedbackDisplay) {
            ...
            // Apply class for potential styling, though text content is primary feedback
            feedbackDisplay.className = `gtn-feedback-${type}`;
        }
    }

    // ** UPDATED ** Get a random pre-defined response based on persona
    ...
    // ** UPDATED ** Handle API call, persona-aware prompt
    async function _fetchApiResponse(resultType, guess) {
        ...
        const personaPromptPart = (currentPersonaInGame === 'Kana')
            ? `You are Kana, a sly, sarcastic catgirl playing 'Guess My Number' (${MIN_NUM}-${MAX_NUM}) with ${currentUserName}.`
            : `You are Mika, a playful, teasing, possessive catgirl playing 'Guess My Number' (${MIN_NUM}-${MAX_NUM}) with ${currentUserName}.`;

        const prompt = `${personaPromptPart} They have ${guessesLeft} guesses left. Situation: ${situation} Give a short (1-2 sentences), in-character response to ${currentUserName}. ${currentPersonaInGame === 'Kana' ? 'Include sarcasm or dry wit.' : 'Include cat noises or actions (*giggle*, *purr*, nyaa~).'}`;
    ...
    function _endGame(win) {
        ...
        // Final feedback based on win/loss handled by handleGuess's call to _sendMessage
        if (win) {
             _updateFeedbackDisplay(`${currentPersonaInGame === 'Kana' ? 'Correct.' : 'You got it!'} The number was ${targetNumber}!`, 'win');
        } else {
             _updateFeedbackDisplay(`${currentPersonaInGame === 'Kana' ? 'You lose.' : 'Out of guesses!'} The number was ${targetNumber}!`, 'lose');
        }
    }

    // ** UPDATED ** Persona-specific messaging on new game
    function _startNewGame() {
        ...
        _updateFeedbackDisplay(`${currentPersonaInGame === 'Kana' ? 'New number.' : 'I\'m thinking of a new number'} between ${MIN_NUM} and ${MAX_NUM}...`, 'info');
        if (hotColdIndicator) {
            hotColdIndicator.textContent = `${currentPersonaInGame === 'Kana' ? 'Guess.' : 'Make your first guess!'}`;
    ...
    async function handleGuess() {
        ...
        let resultType = 'invalid'; // Default
        ...
        if (isNaN(guess) || guess < MIN_NUM || guess > MAX_NUM) {
            ...
            guessesLeft--;
            guessCount++; // Increment valid guess count for API frequency
            _updateGuessesLeftDisplay();
            _updateHotColdIndicator(guess);
        ...
        // Only call API for valid guesses (high, low, win, lose)
        const shouldCallApi = (resultType !== 'invalid' && resultType !== 'repeat');
        ...
        // If API call wasn't attempted/failed, or if it's invalid/repeat, get predefined
        if (!responseMessage) {
    ...
    function init(_gameUiContainer, _messageCallback, _apiCaller, userName, persona) {
        ...
        instructions.textContent = `Guess the number between ${MIN_NUM} and ${MAX_NUM}. You have ${MAX_GUESSES} tries.`;
        instructions.style.marginBottom = '10px';
        instructions.style.textAlign = 'center'; // Center instructions
        ...
        guessInput.className = 'gtn-input'; // Add class for styling from index.html

        guessButton = document.createElement('button');
        guessButton.id = 'gtn-submit';
        guessButton.textContent = 'Guess!';
        guessButton.className = 'rps-choice-button gtn-submit'; // Reuse style, add specific if needed
        ...
        const feedbackContainer = document.createElement('div'); // Container for centering
        feedbackContainer.style.textAlign = 'center';

        feedbackDisplay = document.createElement('div');
        feedbackDisplay.id = 'gtn-feedback';
        // Styles applied via CSS class in index.html
        ...
        // Styles applied via CSS class in index.html
        feedbackContainer.appendChild(hotColdIndicator);

        guessesLeftDisplay = document.createElement('div');
        guessesLeftDisplay.id = 'gtn-guesses-left';
        // Styles applied via CSS class in index.html
        feedbackContainer.appendChild(guessesLeftDisplay);
        ...
        newGameButton.textContent = `${currentPersonaInGame === 'Kana' ? 'Again.' : 'Play Again? ♡'}`; // Persona-specific text
        newGameButton.className = 'rps-choice-button gtn-new-game'; // Reuse style
        ...
        // Append directly to gameUiContainer for centering if needed, or keep in feedbackContainer
        gameUiContainer.appendChild(newGameButton); // Centered by default if parent is centered
